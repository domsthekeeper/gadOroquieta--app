...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="agriculture"):
    try:
...
def display():
    st.set_page_config(page_title="Agriculture & Fisheries Office Dashboard", layout="wide")
    st.title(":bar_chart: City Agriculture & Fisheries Office")

    # Load data
    df = load_data("agriculture")
    if df is None:
        st.stop()
