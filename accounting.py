...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="accounting"):
    try:
...
def display():
    st.set_page_config(page_title="City Accounting Office Dashboard", layout="wide")
    st.title(":bar_chart: City Accounting Office ")

    # Load data
    df = load_data("accounting")
    if df is None:
        st.stop()
