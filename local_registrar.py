...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="local_registrar"):
    try:
...
def display():
    st.set_page_config(page_title="Local Civil Registrar’s Office Dashboard", layout="wide")
    st.title(":bar_chart: Local Civil Registrar’s Office ")

    # Load data
    df = load_data("local_registrar")
    if df is None:
        st.stop()
