...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="general_services"):
    try:
...
def display():
    st.set_page_config(page_title="General Services Office Dashboard", layout="wide")
    st.title(":bar_chart: City General Services Office ")

    # Load data
    df = load_data("general_services")
    if df is None:
        st.stop()
