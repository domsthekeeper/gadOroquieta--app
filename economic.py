...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="economic"):
    try:
...
def display():
    st.set_page_config(page_title="Economic Enterprise & Development Office Dashboard", layout="wide")
    st.title(":bar_chart: City Economic Enterprise & Development Office ")

    # Load data
    df = load_data("economic")
    if df is None:
        st.stop()
