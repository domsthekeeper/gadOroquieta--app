...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="city_mayor"):
    try:
...
def display():
    st.set_page_config(page_title="City Mayor Dashboard", layout="wide")
    st.title(":bar_chart: Office of the City Mayor ")

    # Load data
    df = load_data("city_mayor")
    if df is None:
        st.stop()
