...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="administrator"):
    try:
...
def display():
    st.set_page_config(page_title="Office of the Administrator Dashboard", layout="wide")
    st.title(":bar_chart: Office of the Administrator")

    # Load data
    df = load_data("administrator")
    if df is None:
        st.stop()
