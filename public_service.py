...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="public_service"):
    try:
...
def display():
    st.set_page_config(page_title="Office of the City Public Service Dashboard", layout="wide")
    st.title(":bar_chart: Office of the City Public Service ")

    # Load data
    df = load_data("public_service")
    if df is None:
        st.stop()
