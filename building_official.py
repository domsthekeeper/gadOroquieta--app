...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="building_official"):
    try:
...
def display():
    st.set_page_config(page_title="Office of the Building Official Dashboard", layout="wide")
    st.title(":bar_chart: Office of the Building Official ")

    # Load data
    df = load_data("building_official")
    if df is None:
        st.stop()
