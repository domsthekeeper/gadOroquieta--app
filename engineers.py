...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="engineer"):
    try:
...
def display():
    st.set_page_config(page_title="City Engineer’s Office Dashboard", layout="wide")
    st.title(":bar_chart: City Engineer’s Office ")

    # Load data
    df = load_data("engineer")
    if df is None:
        st.stop()
