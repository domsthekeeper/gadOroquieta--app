...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="city_vice_mayor"):
    try:
...
def display():
    st.set_page_config(page_title="Vice Mayor Dashboard", layout="wide")
    st.title(":bar_chart: City Vice Mayor Office ")

    # Load data
    df = load_data("city_vice_mayor")
    if df is None:
        st.stop()
