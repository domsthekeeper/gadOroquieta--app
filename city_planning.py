...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="city_planning"):
    try:
...
def display():
    st.set_page_config(page_title="City Planning & Development Coordinator Dashboard", layout="wide")
    st.title(":bar_chart: Office of the City Planning and Development Coordinator ")

    # Load data
    df = load_data("city_planning")
    if df is None:
        st.stop()
