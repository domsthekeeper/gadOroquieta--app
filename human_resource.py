...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="human_resource"):
    try:
...
def display():
    st.set_page_config(page_title="Human Resource Management Office Dashboard", layout="wide")
    st.title(":bar_chart: Human Resource Management Office ")

    # Load data
    df = load_data("human_resource")
    if df is None:
        st.stop()
