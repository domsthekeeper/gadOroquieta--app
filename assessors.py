...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="assessors"):
    try:
...
def display():
    st.set_page_config(page_title="City Assessors Office Dashboard", layout="wide")
    st.title(":bar_chart: City Assessors Office ")

    # Load data
    df = load_data("assessors")
    if df is None:
        st.stop()
