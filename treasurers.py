...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="treasurers"):
    try:
...
def display():
    st.set_page_config(page_title="City Treasurer’s Office Dashboard", layout="wide")
    st.title(":bar_chart: City Treasurer’s Office ")

    # Load data
    df = load_data("treasurers")
    if df is None:
        st.stop()
