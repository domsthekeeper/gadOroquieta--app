...
# -------------------- CACHE DATA LOADING --------------------
@st.cache_data
def load_data(sheet_name="disaster"):
    try:
...
def display():
    st.set_page_config(page_title="Disaster Risk Reduction Management Office Dashboard", layout="wide")
    st.title(":bar_chart: City Disaster Risk Reduction Management Office ")

    # Load data
    df = load_data("disaster")
    if df is None:
        st.stop()
